...
class IndexedDBAdapter {
   ...
   * @param {Note} note - Note object (id optional for new notes)
   * @returns {Promise<Note>} Note with id populated
   */
  async saveNote(note) {
    const db = await this.openDB();
    return new Promise((resolve, reject) => {
      ...
      // Use add() for new notes (auto-increment), put() for updates
      const request = note.id ? store.put(note) : store.add(note);
      
      request.onsuccess = () => {
        if (!note.id) {
          note.id = request.result; // Get auto-generated ID
        }
        resolve(note);
      };
      request.onerror = () => reject(request.error);
  ...
  async addTemplate(text) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const store = tx.objectStore('templates');
      const request = store.add(template); // Use add() for auto-increment
      
      request.onsuccess = () => {
        template.id = request.result; // Get auto-generated ID
        resolve(template);
      };
      request.onerror = () => reject(request.error);
    });
